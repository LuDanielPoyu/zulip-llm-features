...
const topic_title_suggestion = mock_esm("../src/topic_title_suggestion", {
    request_topic_title_suggestion: noop,
});
...
test_ui("send_message_success", ({override, override_rewire}) => {
    ...
    const topic_title_suggestion_message_ids = [];

    override(topic_title_suggestion, "request_topic_title_suggestion", (message_id) => {
        topic_title_suggestion_message_ids.push(message_id);
    });
    ...
    assert.deepEqual(topic_title_suggestion_message_ids, [12]);
    ...
    assert.ok(reify_message_id_checked);
    assert.ok(draft_deleted);
    assert.deepEqual(topic_title_suggestion_message_ids, [12, 12]);
    ...
    compose.send_message_success(request, {id: 12});
    assert.ok(reify_message_id_checked);
    assert.ok(draft_deleted);
    assert.deepEqual(topic_title_suggestion_message_ids, [12, 12]);
});
